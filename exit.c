...
//for our policy
//****************************clean policy stack*******************************/
static void exit_policy_stack(struct task_struct * task){
	printk("In exit_policy_stack\n"); //DEBUG
    struct list_head* curr;
    p_policy_inst curr_policy_inst;
    //iterate through list
    list_for_each(curr, &task->policy_stack_head)
    {
		printk("In list_for_each 1\n"); //DEBUG
        curr_policy_inst = list_entry(curr,policy_inst,list_pointers);
		printk("In list_for_each 2\n"); //DEBUG
        if (curr_policy_inst != NULL){
			printk("In list_for_each 3\n"); //DEBUG
			//delete list_head struct
			printk("In list_for_each 6\n"); //DEBUG
			list_del(curr); //?maybe I need to delete curr instead//
			printk("In list_for_each 7\n"); //DEBUG
			kfree(curr_policy_inst);
			printk("after kfree\n"); //DEBUG
		}
		printk("In list_for_each 8\n"); //DEBUG
    }
	printk("In list_for_each 9\n"); //DEBUG
}
